...
flags.DEFINE_float('keep_threshold', 0.2, 'prob of keeping for less than 0.1 angles')
# flags.DEFINE_integer('new_cols', 64, 'number of pixels horizontal')
# flags.DEFINE_integer('new_rows', 64, 'number of pixels vertical')

driving_log_files = []
driving_log_files.append("./data")   # Udacity data
		...
		for i in range(batch_size):
			...
			while keep:
				...
				if abs(y_steer) < 0.1:
					...
					if pr_val < FLAGS.keep_threshold:
						keep = False
				else:
					keep = False
			X.append(img)
...
train_generator = generator_train(train_samples, batch_size = FLAGS.batch_size)
...
# print(math.ceil(len(train_samples)/FLAGS.batch_size))

# model.compile(optimizer='adam',loss='mse')
# model.fit_generator(train_generator, 
# 	steps_per_epoch = math.ceil(len(train_samples)/FLAGS.batch_size), 
# 	epochs = FLAGS.epochs,
# 	verbose = 1, 
# 	validation_data = valid_generator, 
# 	validation_steps = math.ceil(len(valid_samples)/FLAGS.batch_size))
# model.save('modelV1_0.h5')
